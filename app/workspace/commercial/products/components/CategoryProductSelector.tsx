...
import { FiChevronDown, FiEdit3, FiTag, FiBox } from "react-icons/fi";
    ...
    <div className="space-y-6">
      ...
      <div className="relative w-full max-w-xs">
        ...
        >
          ...
          <FiChevronDown
            className={`transition-transform duration-200 text-slate-400 text-lg ${open ? "rotate-180" : "rotate-0"}`}
          />
        </div>
      ...
      {products.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-10 bg-luminiBrandBlue border border-slate-800 rounded-2xl text-center">
          ...
          <p className="text-sm font-medium text-slate-00">No hay productos en esta categoría</p>
          <p className="text-xs text-slate-500 mt-0.5">Intenta seleccionar otra categoría o añade uno nuevo.</p>
        </div>
      ) : (
          ...
          {products.map((prod) => (
            ...
            >
              <div className="space-y-1.5 pr-2">
                <p className="font-semibold text-sm tracking-wide">{prod.name}</p>
                {prod.description && (
                  <p className="text-xs text-slate-300 line-clamp-1">{prod.description}</p>
                )}
                <p className="text-base font-bold text-pink-400 pt-1">$ {prod.salePrice}</p>
              </div>
              <button
                onClick={() => handleEditClick(prod)}
                className="px-3 py-1.5 bg-pink-600 hover:bg-pink-600 hover:text-white border border-slate-700 hover:border-pink-500 rounded-xl text-xs font-medium transition-all flex items-center gap-1.5 shrink-0 shadow-sm"
              >
            ...
            <div className="space-y-3.5 pt-1">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-slate-100 text-sm focus:border-pink-500 focus:ring-1 focus:ring-pink-500 outline-none transition-all placeholder:text-slate-600"
                  placeholder="Nombre"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  onFocus={() => setForm({ ...form, salePrice: 0 })}
                  className="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-slate-100 text-sm focus:border-pink-500 focus:ring-1 focus:ring-pink-500 outline-none transition-all placeholder:text-slate-600"
